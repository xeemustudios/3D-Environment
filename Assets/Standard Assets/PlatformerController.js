...
class PlatformerControllerMovement {
    ...
    var alwaysRun = false;
    var alwaysMoveRight = false;
    var overallSpeedMultiplier = 1.0;
...
public function setOverallSpeedMultiplier(val:float)
{
	movement.overallSpeedMultiplier = val;
}

function UpdateSmoothedMovementDirection () {	
	var h = Input.GetAxisRaw ("Horizontal");

    if (movement.alwaysMoveRight)
        h = 1.0;   
	...
	if (controller.isGrounded) {
		...
		// Pick speed modifier
		if ((Input.GetButton ("Fire2") && canControl) || movement.alwaysRun)
			targetSpeed *= movement.runSpeed;
		else
			targetSpeed *= movement.walkSpeed;

        targetSpeed *= movement.overallSpeedMultiplier;
